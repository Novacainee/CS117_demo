...
// Vietnam geographical boundaries check
...
function extractEnvironmentalFeatures(lat: number, lon: number): {
  ...
  // Simple spatial model of Vietnam:
  // 1. Red River Delta (around 20.5-21.5, 105.5-106.5) -> Low, flat
  // 2. Mekong Delta (around 9.5-10.5, 105.0-106.5) -> Low, flat
  // 3. Northwest Mountains (around 21.5-23.0, 102.5-104.5) -> High, steep
  // 4. Truong Son Mountain Range (border with Laos, 15.0-20.0, 105.0-107.0) -> High, steep forest
  // 5. Tay Nguyen Highlands (around 12.0-14.5, 107.5-108.5) -> Plateau
  ...
  // Simple distance to line segments representing the Truong Son range
  const pts = [
    [19.5, 104.5],
    [18.2, 105.3],
    [16.5, 107.2],
  ...
  elevation = Math.max(elevation, 5); // Must be above sea level for Vietnam land
  ...
  // NDVI (higher in forested/mountainous areas, lower in deltas/plains)
  let ndvi = 0.15; // default urban/plain
  if (elevation > 150) {
  ...
  // Temperature (decreases with latitude and elevation - Lapse rate ~ 0.0065°C per meter)
  // Base temperature at sea level varies from North (22°C) to South (28°C)
  const baseTemp = 28.0 - (lat - 8.5) * 0.3; // Northern latitudes are cooler
  ...
  // Distance to road (forest areas are remote, delta areas are close to roads)
...
function gaussianMatch(value: number, minPref: number, maxPref: number): number {
  ...
  const stdev = range / 3; // 99% of data falls within 3 standard deviations
  if (stdev === 0) return 1;
  return Math.exp(-Math.pow(value - center, 2) / (2 * Math.pow(stdev, 2)));
}

// API: Predict Habitat Suitability
app.post('/api/predict', (req, res) => {
  const { speciesId, latitude, longitude, radiusKm, scenario } = req.body;

  if (latitude === undefined || longitude === undefined || !speciesId) {
  ...
  if (!isValid) {
    return res.json({
      ...
      isSuitable: false,
      cutOffThreshold: 0.5,
      features: { elevation: 0, slope: 0, aspect: 0, aspectSin: 0, aspectCos: 0, ndvi: 0, temperature: 0, distanceToRoad: 0 },
      vifInfo: [],
  ...
  const profiles: Record<string, {
    ...
    distCenter: [number, number]; // main cluster coordinate
    distWeight: number; // sensitivity to core distribution zone
  }> = {
    sao_la: {
      ...
      prefNdvi: [0.65, 0.95],
      prefSlope: [15, 45],
      distCenter: [17.5, 106.1], // Truong Son
      distWeight: 0.5,
    },
    voi_asia: {
    ...
    vooc_cha_va: {
      ...
      prefNdvi: [0.6, 0.9],
      prefSlope: [10, 35],
      distCenter: [16.1, 108.2], // Son Tra / Da Nang
      distWeight: 0.6,
    },
    vuon_soc_den: {
      ...
      prefNdvi: [0.7, 0.9],
      prefSlope: [20, 50],
      distCenter: [22.8, 106.5], // Cao Bang
      distWeight: 0.8,
    },
    ho_dong_duong: {
      ...
      prefNdvi: [0.6, 0.9],
      prefSlope: [10, 30],
      distCenter: [22.3, 102.3], // Muong Nhe
      distWeight: 0.4,
    },
  };

  const profile = profiles[speciesId] || profiles.sao_la;
  ...
  const dist = Math.sqrt(Math.pow(latitude - profile.distCenter[0], 2) + Math.pow(longitude - profile.distCenter[1], 2));
  const distMatch = Math.exp(-dist / (profile.distWeight * 4)); // decays over distance

  // Combine suitability
  let baseScore = (matchElev * 0.3) + (matchTemp * 0.2) + (matchNdvi * 0.25) + (matchSlope * 0.15) + (distMatch * 0.1);
  baseScore = Math.max(0, Math.min(1, baseScore));

  // Determine dynamic threshold based on scenario
  let cutOffThreshold = 0.5; // default
  if (scenario === 'CONSERVATION') {
    cutOffThreshold = 0.45; // lower threshold to raise Recall (prioritize conservation)
  } else if (scenario === 'PATROL') {
    cutOffThreshold = 0.60; // higher threshold to raise Precision (optimize budget)
  }

  // Adjust score slightly based on search radius (larger radius brings slight smoothing/averaging)
  if (radiusKm > 30) {
    baseScore = baseScore * 0.95 + 0.02;
  }

  // Ensure score is nicely formatted
  const suitabilityScore = parseFloat(baseScore.toFixed(4));
  const isSuitable = suitabilityScore >= cutOffThreshold;

  // Simulate VIF Multicollinearity information
  // Temperature and Elevation are highly correlated in physics (lapse rate), VIF should be high!
  // NDVI and Elevation are moderately correlated. Aspect has low VIF.
  const vifInfo = [
    { featureName: 'Elevation', vif: 24.3, status: 'ACCEPTED' as const },
    { featureName: 'Temperature', vif: 25.1, status: 'REJECTED' as const }, // Over VIF limit -> dropped in pipeline
...
// Serve Vite or static build depending on production mode
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[HabitatSuitabilityServer] Server is running on http://localhost:${PORT}`);
  });
}
